...
// Ref returns a SecretRef for this Secret.
...
// Typed resolves the secret and returns a typed secret struct based on the "type" field.
...
// SecretAWS contains AWS credentials.
...
// SecretGCP contains Google Cloud Platform credentials.
type SecretGCP struct {
	APIKey      string `json:"api_key"`
	Credentials string `json:"credentials"` // Credentials file as JSON.
}

// SecretAzure contains Azure credentials.
type SecretAzure struct {
	AccountName string `json:"account_name"`
	AccountKey  string `json:"account_key"`
}

// SecretGitHubApp contains GitHub App authentication credentials.
...
// SecretSSHKey contains SSH key authentication credentials.
...
// SecretBasicAuth contains HTTP basic authentication credentials.
...
// Client returns an HTTP client configured with basic authentication.
...
// SecretTokenAuth contains bearer token authentication credentials.
type SecretTokenAuth struct {
	BearerToken string `json:"token"` // Bearer token for HTTP authentication.
}

// Client returns an HTTP client configured with bearer token authentication.
...
// SecretPassword contains simple password authentication credentials.
type SecretPassword struct {
	Password string `json:"password"` // Password for authentication.
}

// SecretTLSCert contains TLS certificate bundle.
...
// ToConfig converts the secret to a tls.Config.
...
// decode decodes a map into a struct using mapstructure with json tags.
...
// wrappedClient creates an HTTP client with a custom request wrapper.
...
// crt is a custom round tripper that wraps requests.
type crt struct {
